...
func NewLMuxer(options Options) *LMuxer {
	tmp := LMuxer{Options: options, ClientACK: time.NewTimer(time.Second * 20), StreamACK: time.NewTimer(time.Second * 20), streams: make(map[int8]*Stream)}
	//go tmp.WaitCloser()
	return &tmp
}
func (element *LMuxer) NewPeerConnection(configuration webrtc.Configuration) (*webrtc.PeerConnection, error) {
...
func (element *LMuxer) Close() error {
	element.stop = true
	if element.pc != nil {
